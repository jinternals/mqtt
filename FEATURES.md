...
| **Acyclic package layering** | `support ← core ← annotation ← listener`, with `health` and `autoconfigure` on top. Nothing depends on `autoconfigure`, so the library is usable without Boot's auto-config. `MqttConnection` needs what the annotation scan found, so `core` declares `MqttSubscriptionSource` and `MqttListenerRegistry` implements it — dependency inversion rather than a cycle. |
...
| **Silent denial** | A forbidden publish returns `PUBACK RC:0` on MQTT 3.1.1 — indistinguishable from success — and `RC:135` (`0x87 Not authorized`) on MQTT 5. Both measured. The bridge hop is 3.1.1, so it is the one connection that cannot be told. |
| **Where ACL is enforced** | WRITE at publish; READ at delivery, per subscriber. A subscription to an unreadable topic is *granted* (SUBACK 0) and then never fed — so a silent feed can mean a missing grant rather than a missing publisher. |
...
| **MQTT 5 on the bridge hop** | Investigated and rejected. Measured across four brokers: mosquitto and EMQX send no Session Expiry Interval (EMQX also randomises its client id per restart); NanoMQ does with `conn_properties`, and delivers a command queued through an outage. But NanoMQ's *outbound* bridge queue drops messages when the link is down — 5 of 5 telemetry lost with the documented `bridges.mqtt.cache` spool configured, which never wrote a file; the `-full` image additionally segfaults on that path, on both arm64 and amd64. It would fix commands and break telemetry. See `docs/mqtt5-bridge-durability.md`. |
