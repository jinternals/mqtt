...
| **MQTT 5 on the bridge hop** | Reachable but not free. Measured across four brokers: mosquitto and EMQX send no Session Expiry Interval (EMQX also randomises its client id per restart); NanoMQ does when given `conn_properties.session_expiry_interval`, confirmed by delivering a command queued through an outage. The cost is porting TLS, credentials and per-site ACLs to a second config language. See `docs/mqtt5-bridge-durability.md`. |
