...
flowchart LR
    subgraph S1["site1-lan · fulfilment centre"]
        ...
        EB1[("edge-broker-site1<br/><small>TLS · auth · ACL · disk spool</small>")]
        EA1 -->|"mqtts 8883<br/><small>user: edge-app<br/>scoped to sites/site1/**</small>"| EB1
    end

    subgraph S2["site2-lan · fulfilment centre"]
        direction TB
        EA2["edge-app-site2<br/><small>arm-11 · humanoid-12</small>"]
        EB2[("edge-broker-site2<br/><small>TLS · auth · ACL · disk spool</small>")]
        EA2 -->|"mqtts 8883<br/><small>user: edge-app<br/>scoped to sites/site2/**</small>"| EB2
    end

    subgraph W["cloud"]
        direction TB
        CB[("cloud-broker<br/><small>TLS · auth · per-site ACL</small>")]
        CA["cloud-app<br/><small>REST :8080</small>"]
        CB <-->|"mqtts 8883<br/><small>user: cloud-app</small>"| CA
    end

    EB1 ===>|"bridge · mqtts · dials out<br/><small>user: bridge-site1 — sites/site1/** only</small><br/><b>the only unreliable hop</b>"| CB
    EB2 ===>|"bridge · mqtts · dials out<br/><small>user: bridge-site2 — sites/site2/** only</small>"| CB
...
Every hop is TLS against a private CA, every connection authenticates
(`allow_anonymous false`), and every credential is confined by an ACL to the
subtree it has business in.
...
| `bridge-site2` | `sites/site2/command/+/req` | `sites/site2/{telemetry,health,command/+/res,bridge/state}` |
| `cloud-app` | `sites/+/{telemetry,health,command/+/res,bridge/state}`, `$SYS/#` | `sites/+/command/+/req`, `dlq/cloud-service` |
| `observer` | `sites/#`, `dlq/#`, `$SYS/#` | *nothing* |
...
- **A site cannot write its own commands.** The bridge may *read*
  ...
  the whole subtree would let a site inject commands addressed to its own robots,
  bypassing the cloud and every authorisation check that lives there.

The `observer` account the topic browser uses is read-only and separate from both
